...
enum class Pixel {
    EMPTY, WALL,
    GUARD_UP, GUARD_DOWN, GUARD_LEFT, GUARD_RIGHT, GUARD_PATH
}

fun Pixel.isGuard(): Boolean = when (this) {
...
class Day6 : Day {
    ...
    class Board(var board: List<MutableList<Pixel>>) {
        ...
        fun placeBlockInEachPath(path: List<Pair<Int, Int>>): Flow<Board> = flow {
    ...
    fun createBoard(s: String): Board = Board(s.split("\n").map { line ->
        line.map { char ->
            when (char) {
                '.' -> Pixel.EMPTY
                '#' -> Pixel.WALL
    ...
    fun solvePart1(s: String): Int = createBoard(s).let {
    ...
    val moveBoardRD = DeepRecursiveFunction<Pair<Board, Position>, Board> {
        (rboard, pos) ->
    ...
    val moveBoardR = DeepRecursiveFunction<Board, Board> {
            rboard ->
        val currentPosition = getGuardPosition(rboard.board)
        val nextPosition = rboard.nextPosition(currentPosition)
        if (
    ...
    fun solvePart2(s: String): Int = runBlocking {
...
fun <E> List<MutableList<E>>.set(index: Pair<Int, Int>, s: E): Unit {
    val (i, j) = index
    this[i][j] = s
}
