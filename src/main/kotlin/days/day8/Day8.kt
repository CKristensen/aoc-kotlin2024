...
class Day8 : Day {
    ...
    fun digestString(s: String): List<Location> =
        buildList {
            s.split('\n').mapIndexed {
                    y, row ->
                row.mapIndexed { x, value -> add(Location(x, y, value)) }
            }
        }

    fun antennaTypes(input: List<Location>): Set<Char> = buildSet { input.filter { it.antenna() }.map { add(it.value) } }

    fun antennaLocations(
        input: List<Location>,
        antennaType: Char,
    ): List<Location> = input.filter { it.antenna() && it.value == antennaType }
    ...
    ): Boolean = location.x in 0 until mapSize.first && location.y in 0 until mapSize.second

    fun run(
        input: String,
        calculateAntiNodes: (Location, Location) -> List<Location>,
    ) = buildSet {
